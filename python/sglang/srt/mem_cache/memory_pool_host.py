...
class KeyeIndexerPoolHost(HostKVCache):
    """Host-side Keye indexer K buffer (FP8 paged). Standalone class mirroring NSAIndexerPoolHost.

    Follows the anchor pattern: attaches to an anchor MHATokenToKVPoolHost for
    main KV, handles indexer K buffer independently. Supports layer_first and
    page_first layouts with kernel/direct IO transfer.
    """

    device_pool: KeyeTokenToKVPool
    ...
    def __init__(
        ...
        self.index_head_dim = device_pool.index_head_dim
        self.indexer_quant_block_size = device_pool.quant_block_size
        self.indexer_dtype = KeyeTokenToKVPool.index_k_with_scale_buffer_dtype
        self.indexer_size_per_token = (
            self.index_head_dim
            + self.index_head_dim // self.indexer_quant_block_size * 4
        )
        ...
        if requested_bytes > available_bytes:
            raise ValueError(
                f"Not enough host memory for Keye indexer hierarchical cache. "
                f"Requesting {requested_bytes / 1e9:.2f} GB but only have "
                f"{available_bytes / 1e9:.2f} GB free."
            )
        logger.info(
            "Allocating %.2f GB host memory for Keye indexer (layout=%s).",
            requested_bytes / 1e9,
            layout,
        )
        self.init_kv_buffer()
    ...
    def _get_indexer_page_indices(self, host_indices, device_indices):
        ...
        if host_indices.numel() % self.page_size != 0:
            raise ValueError(
                "Index buffer transfer expects page-aligned indices for Keye."
            )
        host_page_indices = (
            host_indices.reshape(-1, self.page_size)[:, 0] // self.page_size
        )
    ...
    def get_page_buffer_meta(self, indices):
        ...
        for i in range(0, len(indices), self.page_size):
            page_index = int(indices[i]) // self.page_size
            ptr_list.append(base_ptr + page_index * page_stride_bytes)
        element_size_list = [page_stride_bytes] * len(ptr_list)
        return ptr_list, element_size_list
